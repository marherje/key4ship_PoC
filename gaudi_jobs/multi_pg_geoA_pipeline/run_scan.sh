#!/bin/bash
#
# Reconstruction pipeline for the geoA geometry variant.
# Runs overlay + digitize + tracking over every geoA multi-PG sample
...
for pair in mu+:mu_pi e+:e_pi; do
  ...
  for a in $angles; do
    f="$data/output_geoA_QGSP_BERT_SND_${p1}_10GeV_pi+_5GeV_angle_${a}.edm4hep.root"
    if [ ! -f "$f" ]; then echo "SKIP (no existe): $f"; continue; fi
    echo "=== geoA ${tag} angle=${a}"
